...
class AppCategories extends React.Component {
  ...
  tableColumns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <span>
          {record.orderDelete ? <span>deleted</span> : <span>not deleted</span>}
        </span>
      ),
    },
    {
...

    {
      ...
      render: (text, record) => (
        <span style={{ display: "flex", flexDirection: "row" }}>
          {record.orderDetail.map((value, index) => (
            <span>
              {console.log(value)}
              <img
                style={{ width: "50px" }}
                src={`${Url}/uploads/${value[0].image}`}
              />
    ...
    },
    {
      ...
      render: (text, record) => (
        <span>
          ...
          <a onClick={() => this.props.dispatch(Deleteorders(record._id))}>
            Delete
          </a>
        </span>
      ),
  ...
  render() {
    ...
    return (
      <div>
        <Row>
          <Col span={24}>
            <Typography.Title>Leader</Typography.Title>
          </Col>
          <Col>
            <Divider />
          </Col>
