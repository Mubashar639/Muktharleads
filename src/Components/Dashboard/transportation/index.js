...
import {
  GetTranport,
  DeleteTranport,
} from "../../../Redux/Epics/transportation";
...
class AppCategories extends React.Component {
  ...
  tableColumns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <span>
          {record.pickUpLocation.map((value) => (
            <a> {value} </a>
          ))}
        </span>
      ),
    },
    {
      ...
      render: (text, record) => (
        <span>
          {record.facilities.map((value, index) => (
            <a> {index + 1 + " " + value + "  "} </a>
          ))}
        </span>
      ),
    },
    {
      ...
      render: (text, record) => (
        <span>
          {record.operationDays.map((value, index) => (
            <a> {index + 1 + " " + value + "  "} </a>
          ))}
        </span>
      ),
    ...
    },
    {
      ...
      render: (text, record) => (
        <span>
          ...
          <a onClick={() => this.props.dispatch(DeleteTranport(record._id))}>
            Delete
          </a>
        </span>
      ),
